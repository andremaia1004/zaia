    ...
    return (
        <div className="space-y-6">
            {days.map((day) => {
                ...
                return (
                    <div key={dateKey} className={cn("glass-panel p-4 transition-all duration-300", isToday ? "border-zaia-500/50 ring-1 ring-zaia-500/20 bg-slate-800/60" : "hover:bg-slate-900/60")}>
                        <h3 className={cn("text-lg font-semibold mb-4 capitalize flex items-center gap-2", isToday ? "text-zaia-300" : "text-slate-300")}>
                            {format(day, 'EEEE, dd/MM', { locale: ptBR })}
                            {isToday && <span className="text-xs bg-zaia-600 text-white px-2 py-0.5 rounded-full">Hoje</span>}
                            ...
                            <div className="space-y-3">
                                {dayAppointments.map(app => (
                                    <div
                                        ...
                                        className="group relative flex items-center justify-between bg-slate-800/50 p-3 rounded-lg border border-white/5 hover:border-zaia-500/30 transition-all hover:translate-x-1 cursor-pointer"
                                    >
                                        <div className="flex flex-col">
                                            <span className="font-medium text-white group-hover:text-zaia-200 transition-colors">{app.client?.name || 'Cliente sem nome'}</span>
                                            <div className="flex items-center gap-2 text-sm text-slate-400">
                                                <span>{app.client?.phone}</span>
                                                {app.origin && <span className="text-xs bg-slate-700 px-1.5 rounded text-slate-300">{app.origin}</span>}
                                            </div>
                                        </div>
...
function StatusBadge({ status }: { status: string }) {
    const styles: Record<string, string> = {
        ...
        'FALTOU': 'bg-red-500/20 text-red-300 border-red-500/30',
        'REMARCADO': 'bg-orange-500/20 text-orange-300 border-orange-500/30',
